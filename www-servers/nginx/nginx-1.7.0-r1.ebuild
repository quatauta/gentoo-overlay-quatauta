...
HTTP_FANCYINDEX_MODULE_PV="master"
...
REQUIRED_USE="
	kerberos? ( nginx_modules_http_spnego_auth )
	pcre-jit? ( pcre )
...
src_prepare() {
	...
	if use nginx_modules_http_upstream_check; then
		epatch "${FILESDIR}"/check_1.5.8+.patch
	fi
...
src_configure() {
	...
	if use nginx_modules_http_spnego_auth ; then
		http_enabled=1
		myconf+=" --add-module=${HTTP_SPNEGO_AUTH_MODULE_WD}"
	fi
...
#	fi
	...
	if use nginx_modules_http_spnego_auth; then
		docinto ${HTTP_SPNEGO_AUTH_MODULE_P}
		dodoc "${HTTP_SPNEGO_AUTH_MODULE_WD}"/{ChangeLog,LICENSE,README.md}
	fi
}
